...
def parametric(
    ...
    """
        ...
        :param includes: A list of strings to represent what additional information to return. Only accepts description, covers, tags, genres.
    """
    if not (tag_category or genre_category):
        if title_search_text:
            return titles(title_search_text)
        else:
            raise ValueError("pymoe.ln.search.wlnupdates.parametric: Requires one of tag_category or genre_category.")
